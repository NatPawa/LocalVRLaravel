<template>
    <transition-group
        ...
        v-on:leave="leave"
    >
    <div class="game w-100 h-100 p-5" v-for="game in games"  :key="game.id" :style="{ backgroundImage: 'url( /games/' + game.urlcover + ')' }">
        <div class="w-100 h-100 m-3 flex-column">
            ...
            <div class="logo w-100 d-flex justify-content-center">
                <img :src="'/games/' + game.urllogo" class="img-logo"/>
            </div>
            ...
            <div class="d-flex justify-content-end">
                <button @mouseover="this.showIcon = game.id" @mouseleave="this.showIcon = null" class=" d-flex btn primary-background text-light rounded-pill p-3 justify-center justify-content-center">Jugar <Transition enter-active-class="animate__animated animate__fadeIn animate__fast" leave-active-class="animate__animated animate__fadeOut animate__fast"><i v-if="this.showIcon == game.id" class="fa fa-play ml-1 align-self-center"></i></Transition></button>
            </div>
        </div>
    </div>
  </transition-group>
